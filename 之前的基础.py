...
- 只能包含字母、下换线、数字
- 区分大小写，age和AGE是不同的变量
- Python自带的变量和关键字都不能作为标识符

2. 可以多个变量赋值相同的值
x=y=z=“123”   分别打印出来都是123
3. 也可以同时赋值不同的值
x，y，z=“1”，“2”，“3”

4. 使用print输出变量，可以字符串+字符串组合输出，但是数字+字符串会报错
5. 在函数外部创建的变量为全局变量，在函数内部创建的为局部变量，只能在函数内部使用，若这两的变量名相同，全局变量的值将保留原样。
6. 如果想在函数内部创建全局变量，可以使用global关键词：
Def  函数名（）：
Global x
x=“123”
Print（x） 如果没有定义x为全局变量，直接打印是打印不出来的。但是定义了global，就可以打印123

二、数据类型
1. 使用type()函数获取任何对象的数据类型
x=123  print(type(x))  打印x的数据类型为int
2. list列表和tuple元组的区别
list创建后可变 tuple创建后不可变  执行改变元组数据的代码会报错
list使用[] tuple使用 ()
...
2. float（小数）
3. complex（复数）
可以将一种类型转换成另一种，但是无法将复数转换为其他的数字类型

4. 有个random内置模块用于生成随机数：
导入import random  
print（random.randrange（1,10））#打印显示1-9之间的随机数，随机一个数
5. 如果y=int（2.9） 打印y时，只会打印2

四、字符串
1. 使用三个引号可以给多行字符串赋值
2. 字符串是数组（第一个字符的位置是0）
a=whatyouname  print（a[1])  会打印a字符串的第二个字母h  
裁剪：print（a[2:5]）打印位置2到位置4
负的索引：a[-5:-2]   最后一个数是-1，依次往前数，打印出-5/-4/-3的字符，不包括-2
e是-1
u是-5  所以a[-5:-2]=una
3. 使用len（）打印字符串的长度
4. 使用strip（）可以删除开头和结尾空白的字符
5. lower（）将字符串返回成小写，指打印时调用这个方法
6. upper（）将字符串返回成大写
7. replace（）替换字符，replace（原有的字符，需要替换的字符）
8. split（）拟定一个分隔符，字符串按照分隔符进行拆分，结果使用[AA，bb]
